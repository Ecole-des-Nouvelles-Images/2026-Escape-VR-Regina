...
public class RotaryDialController : MonoBehaviour
{
    ...
    /// <summary>
    /// Fired when a hole has been fully dialled (finger reached max rotation).
    /// Passes the name of the hole that was dialled (e.g. "1", "5", "0").
    /// </summary>
    public event Action<string> OnDigitDialled;
    ...
    private void UpdateDialing()
    {
        ...
        {
            OnDigitDialled?.Invoke(_currentHole.name);
            StartReturnSequence();
        }
    }
